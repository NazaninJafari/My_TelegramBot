...
#  ('e5*', 4) 
#)
@nazanin_bot.message_handler(commands=['music'])
def creat_music(message):
    msg = nazanin_bot.reply_to(message, "make a song... \nexempl: \n('c', 4), ('e', 4), ('g',4), ('c5', -2), ('e6', 8) \nor \n('c*', 4), ('eb', 4), ('g#', 4), ('g*', 2), ('g5', 4), ('g5*', 4), ('r', 4), ('e5', 16), ('f5', 16), ('e5', 16), ('d5', 16), ('e5*', 4)")
    nazanin_bot.register_next_step_handler(msg , music)

def music(message):
    song = (message.text)
    pysynth_b.make_wav(ast.literal_eval(song), fn= "song.wav", leg_stac= .7, bpm= 180)
    newfile = open('song.wav' , 'rb')
    nazanin_bot.send_audio(message.chat.id , newfile)
    #nazanin_bot.send_voice()
